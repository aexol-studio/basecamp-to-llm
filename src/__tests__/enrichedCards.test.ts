...
import type { Comment, Creator, EnrichedCardContext, Step } from '../basecamp-types';
...
function mockStep(
  ...
  title: string,
  completed: boolean,
  options: { assignees?: Creator[]; due_on?: string } = {}
): Step {
  return {
    id,
    status: 'active',
    ...
    type: 'Kanban::Step',
    url: `https://3.basecampapi.com/steps/${id}.json`,
    app_url: `https://3.basecamp.com/steps/${id}`,
    bookmark_url: `https://3.basecamp.com/steps/${id}/bookmark`,
    subscription_url: `https://3.basecampapi.com/steps/${id}/subscription.json`,
    completed,
    ...(options.due_on !== undefined && { due_on: options.due_on }),
    assignees: options.assignees ?? [],
    completion_url: `https://3.basecampapi.com/steps/${id}/completion.json`,
    position: id,
    bucket: { id: 1, name: 'Project A', type: 'Project' },
    creator: mockCreator(1, 'John Doe'),
    parent: {
      id: 123,
      title: 'Test Card',
      type: 'Kanban::Card',
      url: 'https://3.basecampapi.com/cards/123.json',
      app_url: 'https://3.basecamp.com/cards/123',
    },
  };
}
...
describe('Enriched Cards', () => {
  describe('parseAttachments', () => {
    ...
    <img src="https://preview.example.com/image.png">
      ...
      const attachments = parseAttachments(htmlContent);
    ...
    it('should parse multiple attachments', () => {
      ...
      if (!first || !second) throw new Error('Expected two parsed attachments');
      expect(first.filename).toBe('file1.png');
      expect(second.filename).toBe('file2.pdf');
    });
    ...
    <img src="https://preview.3.basecamp.com/preview.png">
      ...
      const attachments = parseAttachments(cardDescription);
  ...
  describe('getEnrichedCard', () => {
    it('should prefer structured attachment download_url over storage.app HTML hrefs', async () => {
      const apiDownloadUrl = 'https://3.basecampapi.com/12345/blobs/abc123/download/spec.pdf';
      ...
      const comment: Comment = {
        ...
        parent: {
          ...
          app_url: 'https://3.basecamp.com/12345/buckets/1/card_tables/cards/456',
        },
        ...
        content_attachments: [
          {
            ...
            preview_url: 'https://preview.3.basecamp.com/12345/blobs/abc123/previews/card',
        ...
        description_attachments: [
          {
            ...
            preview_url: 'https://preview.3.basecamp.com/12345/blobs/cardimage/previews/card',
      ...
      const enriched = await getEnrichedCard(mockClient as unknown as BasecampClient, 12345, 456);
    ...
    it('should format enriched card context as readable text', () => {
      const mockContext: EnrichedCardContext = {
        card: {
          ...
          creator: mockCreator(1, 'John Doe'),
          steps: [
            mockStep(1, 'Step 1', true),
            mockStep(2, 'Step 2', false, {
              assignees: [mockCreator(3, 'Jane')],
              due_on: '2025-11-25',
            }),
          ],
          assignees: [],
      ...
      expect(text).toContain('<p>Test <strong>description</strong></p>');
