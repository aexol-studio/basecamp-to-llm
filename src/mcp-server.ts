...
const pkg = JSON.parse(readFileSync(path.join(__mcp_dirname, '..', 'package.json'), 'utf-8')) as {
  version: string;
};
...
export class BasecampMCPServer {
  ...
  private registerTools(): void {
    ...
    this.registerTool(
      'api_request',
      {
        ...
        inputSchema: apiRequestZodSchema,
      },
      async (args: {
        method: HttpMethod;
        path: string;
    ...
    this.registerTool(
      'sdk_attachments_download',
      {
        description: attachmentDownloadDescription,
        inputSchema: attachmentDownloadZodSchema,
      },
      async (args: { url: string; filename: string | undefined; mimeType: string | undefined }) => {
        try {
          const client = new BasecampClient();
          const result = await downloadAttachment(client, args.url, args.filename, args.mimeType);
  ...
  private async handleAuthenticate(args: { openBrowser?: boolean | undefined }) {
  ...
  private async autoAuthenticate(): Promise<void> {
    ...
    console.error('No valid token found. Opening browser for authentication...');
